...
export default function UploadListItem({ file, actions, thumbnail = false }: Props) {
  ...
  useEffect(() => {
    ...
    if (['done'].includes(file.status!) && format === 'img') {
  ...
  const closeButton = (
    <button
      className="hover:bg-gray-400 ml-auto h-6 w-6 cursor-pointer rounded-full text-center hover:bg-opacity-20"
