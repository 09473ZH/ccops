...
class AgentService:
    """Agent 服务，处理 Agent WebSocket 连接和相关业务逻辑"""
    
    def __init__(self):
        self.logger = logger
    
    async def handle_agent_websocket(self, websocket: WebSocket):
        """处理 Agent WebSocket 连接的主要逻辑"""
        agent_id = None
        ...
        try:
            ...
            # 处理注册
            agent_id = await self._handle_agent_registration(websocket)
            if not agent_id:
                return
    ...
    async def _handle_agent_registration(self, websocket: WebSocket) -> str:
        """处理 Agent 注册流程"""
        try:
            # 等待注册消息
    ...
    async def _handle_agent_messages(self, websocket: WebSocket, agent_id: str):
        """处理 Agent 消息循环"""
        while True:
            try:
                # 接收来自Agent的消息
    ...
    async def get_online_hosts(self):
        """获取在线主机列表"""
        return {
    ...
    async def get_host_agent_status(self, host_id: int):
        """检查指定主机的Agent连接状态"""
    ...
    async def start_host_ssh_session(self, host_id: int, session_data: dict = None):
        """启动指定主机的SSH会话"""
    ...
    async def stop_host_ssh_session(self, host_id: int, session_data: dict = None):
        """停止指定主机的SSH会话"""
        if not agent_manager.is_host_online(str(host_id)):
            raise HTTPException(status_code=404, detail="Host agent not found or offline")
