...
func (s *AlertService) CheckMetrics(metrics *monitor.MetricPoint) error {
	...
	for _, rule := range rules {
		// 检查规则是否适用于当前主机
		if !s.isRuleApplicableToHost(rule, metrics.HostID, hostLabels) {
		...
		for _, r := range rule.Rules {
			...
			// 检查是否在忽略列表中
			if contains(r.IgnoreHosts, metrics.HostID) {
				log.Printf("主机ID %d 在忽略列表中，跳过检查", metrics.HostID)
				continue
			}
...
// isRuleApplicableToHost 检查规则是否适用于指定主机
func (s *AlertService) isRuleApplicableToHost(rule alert.AlertRule, hostID uint64, hostLabels []uint64) bool {
	// 如果规则指定了主机ID列表，检查当前主机是否在列表中
	if len(rule.HostIDs) > 0 {
		return contains(rule.HostIDs, hostID)
	}

	// 如果规则指定了标签ID列表，检查当前主机是否有匹配的标签
	if len(rule.LabelIDs) > 0 {
		for _, labelID := range hostLabels {
			if contains(rule.LabelIDs, labelID) {
				return true
			}
		}
		return false
...
func (s *AlertService) checkDuration(ruleKey string, duration uint64) bool {
	...
	expectedCount := int64(duration / 5) // 假设每5秒一个数据点
	count := s.triggerCount[ruleKey]

	log.Printf("告警持续时间检查: 规则=%s, 当前计数=%d, 需要计数=%d, 开始时间=%v",
		ruleKey, count, expectedCount, s.triggerStartTime[ruleKey])
